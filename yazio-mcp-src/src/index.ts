...
class YazioMcpServer {
  ...
  private extendWaterIntakeSupport(client: Yazio): void {
    ...
    client.user.addWaterIntake = async (entries: YazioAddWaterIntakeOptions): Promise<void> => {
      // @ts-expect-error - Accessing internal auth from yazio client; authenticate() refreshes the token if expired
      const { access_token: token } = await client.auth.authenticate();
  ...
  private setupToolHandlers(): void {
    ...
    this.server.registerTool(
      'log_quick_entry',
      {
        description: 'Log a free-form food entry directly by name and total macros, without needing a product_id from the Yazio database. Use this for restaurant meals, takeout, or homemade dishes where you have estimated calories and macros but no exact product match. The entry appears in the Yazio diary as a "simple product".',
        inputSchema: LogQuickEntryInputSchema,
        annotations: {
          readOnlyHint: false,
          idempotentHint: false,
        },
      },
      async (args: LogQuickEntryInput) => {
        return await this.logQuickEntry(args);
      }
    );
  }
...
- The tool sends: \`[{ date: "2025-12-18 12:00:00", water_intake: 750 }]\` to the API`
  ...
  private async logQuickEntry(args: LogQuickEntryInput) {
    const client = await this.ensureAuthenticated();
    ...
    try {
        ...
        simple_products: [
          {
            ...
            name: args.name,
            nutrients: {
              'energy.energy': args.calories,
              'nutrient.protein': args.protein,
              'nutrient.carb': args.carbs,
              'nutrient.fat': args.fat,
            },
          },
        ],
      };
      ...
      if (!response.ok) {
        const errorText = await response.text();
        throw new Error(`Yazio API returned ${response.status} ${response.statusText}: ${errorText}`);
      }
      ...
      return {
        content: [
          {
            type: 'text' as const,
            text: `Successfully logged quick entry "${args.name}" for ${args.daytime} on ${args.date} (${args.calories} kcal, P ${args.protein}g / C ${args.carbs}g / F ${args.fat}g)`,
          },
        ],
      };
    } catch (error) {
      throw new Error(`Failed to log quick entry: ${error}`);
    }
  }
